...
describe('TransactionService', () => {
    ...
    const mockTransactionDetails = {
        ...
        timeline: [
            { step: 7, status: order_status_types_1.OrderTrackingStatus.PAID, label: 'Paid', completed: true, active: true }
        ],
        payment: {
            baseAmount: 1750,
            qualityBonus: 50,
    ...
    beforeEach(() => {
        ...
        service = new transaction_service_1.TransactionService(mockRepo);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('getEarnings - AC1', () => {
        ...
        it('should throw INVALID_FARMER_ID for zero farmer ID', async () => {
            await expect(service.getEarnings(0)).rejects.toThrow(transaction_service_1.TransactionError);
            await expect(service.getEarnings(0)).rejects.toMatchObject({
                code: transaction_service_1.TransactionErrorCode.INVALID_FARMER_ID
            });
        });
        it('should throw INVALID_FARMER_ID for negative farmer ID', async () => {
            await expect(service.getEarnings(-1)).rejects.toThrow(transaction_service_1.TransactionError);
        });
    ...
    describe('getTransactions - AC2, AC3', () => {
        it('should return paginated transactions with default filter', async () => {
            ...
            const filter = { farmerId: testFarmerId };
    ...
    describe('getTransactionDetails - AC4', () => {
        ...
        it('should throw TRANSACTION_NOT_FOUND for unknown transaction', async () => {
            ...
            await expect(service.getTransactionDetails('UNKNOWN-ID', testFarmerId)).rejects.toMatchObject({
                code: transaction_service_1.TransactionErrorCode.TRANSACTION_NOT_FOUND
            });
        });
    ...
    describe('canDownloadReceipt - AC5', () => {
        ...
        it('should throw error for unknown transaction', async () => {
            mockRepo.getTransactionDetails.mockResolvedValue(null);
            await expect(service.canDownloadReceipt('UNKNOWN-ID', testFarmerId)).rejects.toThrow(transaction_service_1.TransactionError);
        });
    });
});
